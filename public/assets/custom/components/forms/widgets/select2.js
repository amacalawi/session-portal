...
var Select2 = function() {
    ...
    var demos = function() {
        ...
        // tagging support
        if ($('#applicants option').length == 0) {
            $('#applicants').select2({
                placeholder: "Select an applicants",
                tags: true
            });
        } else {
            $('#applicants').select2({
                placeholder: "Select an applicants",
                tags: true
            }).select2('destroy').find('option').each(function () {
                $(this).attr('value', $(this).text());
            }).prop('selected', 'selected').end().select2({
                placeholder: "Select an applicants",
                tags: true,
            }).trigger('change');
        }

        if ($('#inventors option').length == 0) {
            $('#inventors').select2({
                placeholder: "Select an inventors",
                tags: true
            });
        } else {
            $('#inventors').select2({
